...
<div align="center">
	...
	<div>
		<div>
			...
			<p>
				<strong>${memberInfo.MEMBER_ID }</strong>님 안녕하세요<br />
				상품 구매시 10%의 포인트를 적립받으실 수 있습니다.<br />
				<a>가용포인트 :
					<strong><fmt:formatNumber value="${memberInfo.POINT }" pattern="#,###" /> P</strong>
				</a>
			</p>
			</c:if>
...
<!-- 주문내역 -->
<form name="Order_form" action="/grasse/order/orderInsert.do"  onsubmit="return check()">
<div>
	<div align="center">
		<h3>주문내역</h3>
	</div>
	<div align="center">
		<table border="1">
			...
			<tfoot class="rigth">
				<tr>
					<td colspan="8"><span>[기본배송]</span> 상품구매금액
						<strong class="totalPrice">0 원</strong> + 배송비
						<strong class="delivery">0 원</strong> = 합계 :
						<strong><span class="totalPrice">0 원</span></strong>
					</td>
				</tr>
			</tfoot>
			<tbody>
				<c:choose>
						<c:when test="${fn:length(list)>0 }">
							...
							<td class="thumb" align="center">
								<!-- 아이템 링크 확인 -->
								<a href="/grasse/item/detail?ITEM_NO=${row.ITEM_NO }">
								<img width="75" height="75" src="/grasse/resources/upload/${row.IMAGE.split(',')[0] }"/>
								</a>
							</td>
							<td align="center">
								<a href="/grasse/item/detail?ITEM_NO=${row.ITEM_NO }"><strong>${row.NAME }</strong>
								</a>
								<div>[옵션:${row.SIZE}]</div>
							</td>
							<td align="center"> 
								<div>
									<span class="price" value="{row.PRICE}"><strong>
										<fmt:formatNumber value="${row.PRICE }" pattern="#,###"/>원</strong></span>
								</div>
							</td>
							<td align="center">${row.COUNT} 개
								<c:if test="${row.WISHLIST_NO != null }">1</c:if>
							</td>
							<td align="center"><c:if test="${memberInfo.NAME != null }">
							<span class="point" value="${row.PRICE/10*row.COUNT }">
								<fmt:formatNumber value="${row.PRICE/10*row.COUNT }" pattern="#,###"/>P
							</span>
							</c:if>
							</td>
								...
								<c:choose>
									...
									<c:otherwise>
										<span class="totprice" value="${row.PRICE * row.COUNT }">
											<strong><fmt:formatNumber value="${row.PRICE * row.COUNT }" pattern="#,###" />원</strong>
										</span>
									</c:otherwise>
								</c:choose>
							</td>
		...
		<table border="1">
			...
			<tbody class="">
					...
					<tr class="">
						...
						<td>
							<div>
								...
								<input id="sameaddr1" name="sameaddr1" value="F" type="radio">
				...
				<tr>
					...
					<td><input type="text" id="RECIPIENT_NAME" name="RECIPIENT_NAME">
					</td>
				</tr>
				<tr>
					<th scope="row">주소<font color="red">*</font></th>
							<td><input type="text" id="RECIPIENT_ZIPCODE" name="RECIPIENT_ZIPCODE"
								placeholder="우편번호"> <input type="button" value="우편번호"
								onclick="execDaumPostcode()"><br>
								<input type="text" id="RECIPIENT_ADDR1" name="RECIPIENT_ADDR1" placeholder="기본주소">
								<input type="text" id="RECIPIENT_ADDR2" name="RECIPIENT_ADDR2" placeholder="상세주소"><br />
								<input type="text" id="RECIPIENT_EXTRAADDR" name="RECIPIENT_EXTRAADDR" placeholder="참고항목">
							</td>
				</tr>
				<tr>
					<th scope="row">전화번호<font color="red">*</font></th>
					<td>
						<input type="text" id="RECIPIENT_PHONE" name="RECIPIENT_PHONE" placeholder="-없이 번호만 입력해주세요">
			...
			<tr>
				...
				<td>
					<ul>
						<li>총 적립예정 포인트  : 
							<span class="pointSum">
							<strong><fmt:formatNumber value="0" pattern="#,###" /></strong>P</span>
		...
		<table border="1">
			...
			<tbody>
				<tr>
					...
					<td>
						<div>
							<strong><span class="discount">-0 원</span></strong>
							<input type="hidden" name="DISCOUNT2" class="discount2">						
						</div>
					</td>
					<td>
						<div>
							<strong>=</strong><strong><span class="totalSum" class="totalSum">0 원</span></strong>
...
<div align="center">
	<div>
		<div>
			<span>
				<input type="radio" id="무통장입금" name="PAYMENT" value="무통장입금" checked="checked">
			...
			<table border="1">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<select id="DEPOSIT_" name="DEPOSIT_" >
								<option value="-1">::: 선택해 주세요. :::</option>
								<option value="bank:농협:0000-00-000000-00:Grasse">농협:0000-00-000000-00 Grasse</option>
								<option value="bank:국민:000-00-00000-0000:Grasse">국민:000-00-00000-0000 Grasse</option>
							</select>
						</td>
					</tr>
				</tbody>
...
<div align="center">
	...
	<p>
		<input type="checkbox" id="chk_purchase_agreement" name="chk_purchase_agreement[]" value="1">
		<label for="chk_purchase_agreement">결제정보를 확인하였으며, 구매진행에 동의합니다.</label>
	</p>
	<div>
