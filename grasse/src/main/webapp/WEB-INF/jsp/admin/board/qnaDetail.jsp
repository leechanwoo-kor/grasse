...
<body>
	<div class="container">
		<main role="main" class="col-md-12 order-md-1"
			...
			<form name="frm" action="/grasse/admin/qnaUpdate.do" method="post"
				onsubmit="return check()">

				<input type="hidden" class="form-control" id="QNA_NO" name="QNA_NO"
					value="${qnaInfo.QNA_NO }">
							...
							<c:choose>
								<c:when test="${qnaInfo.CATEGORY==0 }">
									<select class="custom-select" id="CATEGORY" name="CATEGORY">
										<option selected value="0">주문 전화</option>
								...
								<c:when test="${qnaInfo.CATEGORY==1 }">
									<select class="custom-select" id="CATEGORY" name="CATEGORY">
										...
										<option selected value="1">배송관련</option>
										<option value="2">포인트 제도</option>
										<option value="3">샘플 안내</option>
										<option value="4">회원관리</option>
									</select>
								...
								<c:when test="${qnaInfo.CATEGORY==2 }">
									<select class="custom-select" id="CATEGORY" name="CATEGORY">
										<option value="0">주문 전화</option>
										<option value="1">배송관련</option>
										<option selected value="2">포인트 제도</option>
										<option value="3">샘플 안내</option>
										<option value="4">회원관리</option>
									</select>
								</c:when>
								<c:when test="${qnaInfo.CATEGORY==3 }">
									<select class="custom-select" id="CATEGORY" name="CATEGORY">
										...
										<option value="1">배송관련</option>
										<option value="2">포인트 제도</option>
										<option selected value="3">샘플 안내</option>
										<option value="4">회원관리</option>
									</select>
								</c:when>
								<c:when test="${qnaInfo.CATEGORY==4 }">
									<select class="custom-select" id="CATEGORY" name="CATEGORY">
										...
										<option value="2">포인트 제도</option>
										<option value="3">샘플 안내</option>
										<option selected value="4">회원관리</option>
									</select>
								</c:when>
							</c:choose>
						</div>
					...
					<div class="col">
						<div class="input-group bg-light">
							<textarea class="form-control" id="CONTENT" name="CONTENT"
								rows="15" style="background-color: #ffffff;">${qnaInfo.CONTENT }</textarea>
						</div>
					</div>
				</div>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="exampleModalLabel">QNA 삭제</h5>
	...
	<script>
		function clickDel(frm) {
			frm.action = "/grasse/admin/qnaDelete.do";
			frm.method = "post";
			frm.submit();
		}
	</script>
